...
function buildMetadata(sample) {
  d3.json("samples.json").then((data) => {
    ...
    Object.entries(result).forEach(([key, value]) => {
      ...
      .property('style',"font-weight: bold");
    });
...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var otu_values = sample_result.sample_values;
    ...
    var xticks = otu_values.map(val => parseInt(val)).slice(0,10);
    var hover = otu_labels.slice(0,10);
    ...
    var gaugeData = [{
      type:'indicator',
      mode:'gauge+number',
      value:parseInt(metadata_result.wfreq),
      gauge:{
        axis:{dtick:2,range:[null,10]},
        bar:{color:'black'},
        steps:[
        {range: [0,2], color:"red"},
